...
const std::string portal_name = "Portal";
const std::string guard_name = "Guard";

int DDoSProtection::start_server(short port, const ConfigParser::ProtectThingConfig& config) {
    std::string target_host = config.hostname;
    ...
    try {
        ...
        Logger::info(std::format("ProtectThing running on port {}", port), guard_name);
        Logger::info(std::format("Forwarding to {}:{}", target_host, target_port), guard_name);
        ...
        while(true) {
            ...
            if(!error_code) {
                ...
                Logger::info(std::format("Client connected: [IP: {}:{}]", client_ip, client_port), guard_name);
            } else {
                Logger::error(std::format("Failed to get remote endpoint: {}", error_code.message()), guard_name);
            }
            ...
            if(error_code) {
                Logger::error(std::format("Error reading client request: {}", error_code.message()), guard_name);
                client_socket.close();
                continue;
            }
            ...
            if(error_code) {
                Logger::error(std::format("Failed to resolve target: {}", error_code.message()), portal_name);

                http::response<http::string_body> error_res{http::status::bad_gateway, req.version()};
                error_res.set(http::field::server, portal_name);
            ...
            tcp::socket target_socket(ioc);
            net::connect(target_socket, results.begin(), results.end(), error_code);

            if(error_code) {
                Logger::error(std::format("Failed to connect to target: {}", error_code.message()), portal_name);
            ...
            if(error_code) {
                Logger::error(std::format("Error writing to target: {}", error_code.message()), portal_name);
                target_socket.close();
                client_socket.close();
                continue;
            }
            ...
            if(error_code) {
                Logger::error(std::format("Error reading from target: {}", error_code.message()), portal_name);
                target_socket.close();
                client_socket.close();
                continue;
            }
            ...
            if(error_code)
                Logger::error(std::format("Error writing to client: {}", error_code.message()), portal_name);
        ...
        Logger::error(std::format("Exception: {}", exception.what()), guard_name);
    }
